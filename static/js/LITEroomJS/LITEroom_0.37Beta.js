...
var LITEroom_Version = "0.3.7 Beta";
...
*
*/
//*** New changes are commented with 3 stars like this
...
AFRAME.registerComponent('lite_button',{
    schema: {
        ...
        fgColor: {default: "#C30D23"}, // foreground color
        // hoverColor: {default: "#C30D23"},// border color when hovered
        hoverFgColor: {default: "#0c0c0c"},// foreground color when hovered
        inactiveFgColor: {default: "#C30D23"}, // foreground color when the button is deactivated
        ...
        labelXPosition: {default: "0"}, // the text on the button
        labelSize: {default: "1"},  // Determines the size of the label. Smaller value gives bigger text.
                                    // Wrap count is the number of characters per line for a given line width
    ...
    init: function(){
        ...
        bg.addEventListener("mouseenter", function (){
            ...
            if(self.data.active){
                ...
                fg.setAttribute("material","color", self.data.hoverFgColor);
            }
        ...
        bg.addEventListener("mouseleave", function (){
            ...
            if(self.data.active){
              fg.setAttribute("material","color", self.data.fgColor);
            }
        });
    ...
    update: function(oldData){
        ...
        var bg = this.bg;
        bg.setAttribute("material", "color", data.active ? data.bgColor : data.inactiveBgColor);
        ...
        var fg = this.fg;
        fg.setAttribute("material", "color", data.active ? data.fgColor : data.inactiveFgColor);
        ...
        label.setAttribute("position", data.labelXPosition + " 0 0.01");
        if(data.visible){
...
AFRAME.registerComponent('lite_round_button',{
    ...
    schema: {
        ...
        inactiveBgColor: {default: "#555555"}, // background color when the button is deactivated
        active: {default: true}, // is the button clickable?
        visible: {default: true}, // show or hide the button
        radius: {default: 0.3},
        ...
        hoverableClass: {default: "hoverable"}, // The class of objects to trigger onmouseenter event when hovered.
        onclick: {default: ""},
        onmouseenter: {default: ""}, // not implemented yet
    },
    init: function(){
        var self = this;
        var container = this.container = document.createElement("a-entity");
    ...
    update: function(oldData){
        ...
        var bg = this.bg;
        bg.setAttribute("material", {color: data.active ? data.bgColor : data.inactiveBgColor, opacity: data.bgOpacity});
        ...
        if(data.active){
            if(data.clickable) bg.classList.add(data.clickableClass);
            if(data.hoverable) bg.classList.add(data.hoverableClass);
            if(data.hoverable) this.fg.classList.add(data.hoverableClass);
        }

        var fg = this.fg;
        fg.setAttribute("material",{color: data.active ? data.fgColor : data.inactiveColor, opacity: data.fgOpacity});
        ...
        if (data.labelType == "image") {
            ...
            var wrapCount = 2 * data.radius * 5.0 / data.labelSize;
    ...
    getSize: function(){
        return {radius: this.data.radius};
    }
});
